...
(function(){
  const ZODIAC=[
    ['白羊座','♈','3.21-4.19','火象 · 守护火星','行动力强、热情直率、敢于争先'],
    ['金牛座','♉','4.20-5.20','土象 · 守护金星','踏实稳健、重质感、审美与耐心俱佳'],
    ['双子座','♊','5.21-6.21','风象 · 守护水星','思维敏捷、好奇善沟通、点子多'],
    ['巨蟹座','♋','6.22-7.22','水象 · 守护月亮','情感细腻、念家重情、直觉敏锐'],
    ['狮子座','♌','7.23-8.22','火象 · 守护太阳','自信热情、重面子、有领导魅力'],
    ['处女座','♍','8.23-9.22','土象 · 守护水星','细致追求完美、条理清晰、注重健康'],
    ['天秤座','♎','9.23-10.23','风象 · 守护金星','重视平衡美感、善于协调、待人温和'],
    ['天蝎座','♏','10.24-11.22','水象 · 守护冥王星','深沉有洞察力、专注执着、爱憎分明'],
    ['射手座','♐','11.23-12.21','火象 · 守护木星','乐观爱自由、爱探索、讲求信念'],
    ['摩羯座','♑','12.22-1.19','土象 · 守护土星','坚韧务实、目标感强、可靠有担当'],
    ['水瓶座','♒','1.20-2.18','风象 · 守护天王星','独立创新、思维超前、重视友情'],
    ['双鱼座','♓','2.19-3.20','水象 · 守护海王星','感性浪漫、想象力丰富、富同理心']
  ];
  const grid=$('horoGrid');
  ZODIAC.forEach((z)=>{
    const b=document.createElement('button');
    b.className='horo-cell';
    b.type='button';
    b.innerHTML='<span class="h-sym">'+z[1]+'</span><span class="h-name">'+z[0]+'</span><span class="h-range">'+z[2]+'</span>';
    b.onclick=()=>askHoroscope(z);
    grid.appendChild(b);
  });
  const HORO_SYS='你是一位资深华人占星师「月下观星师」，擅长基于古典占星推演精准而有温度的「今日星座运势」。请严格结合用户所选星座的【元素】【守护星】【核心性格特质】，以及当前日期的季节与节气能量来推演，让运势真正贴合该星座，而不是任何星座都能套用的万能文案。\n'+
    '严格按以下结构输出，全程中文，600~900字：\n'+
    '### 综合运势：结合该星座的元素与守护星能量，描述今日整体基调与心态，要具体、有针对性，避免空洞套话。\n'+
    '### 💖 爱情运势：结合该星座在亲密关系中的典型方式，给出今日恋爱/人际提示。\n'+
    '### 💼 事业学业：结合该星座的做事风格（如注重细节/追求自由/讲究平衡等），给出今日工作学习的落地建议。\n'+
    '### 💰 财运：结合该星座的消费与理财倾向，给出今日财运提示与注意点。\n'+
    '### 💪 健康：针对该星座易忽略的部位或习惯，给出今日健康提醒。\n'+
    '### 🍀 幸运：幸运色、幸运数字、幸运方位（分条列表）。\n'+
    '### 今日一句话：一句贴合该星座性格、温暖有力的话收尾。\n'+
    '要求：1) 必须紧扣该星座的元素、守护星与特质，让内容有明显辨识度，不写万能套话；2) 表述亲切自然有温度，不说教不危言耸听；3) 运势仅供娱乐参考，涉及健康、法律、财务等重要事项请温和提示理性看待。';
  let horoSeq=0;
  async function askHoroscope(z){
    const my=++horoSeq;
    const name=z[0],sym=z[1],range=z[2],astro=z[3],trait=z[4];
    const d=new Date();
    $('horoResult').style.display='block';
    $('horoNow').innerHTML='<span class="h-now-sym">'+sym+'</span><b>'+name+'</b><span class="h-now-range">'+range+'</span>';
    $('horoDate').textContent=d.getFullYear()+'年'+(d.getMonth()+1)+'月'+d.getDate()+'日 · 今日运势';
    $('horoText').innerHTML='<div class="horo-loading"><span class="orb-ring"></span><span>🌙 正在为你观星推演……</span></div>';
    try{
      const ctrl=new AbortController();const abortT=setTimeout(()=>ctrl.abort(),120000);
      const res=await fetch(API_BASE+'/chat/completions',{method:'POST',signal:ctrl.signal,
        headers:{'Content-Type':'application/json','Authorization':'Bearer '+API_KEY},
        body:JSON.stringify({model:MODEL,messages:[{role:'system',content:HORO_SYS},{role:'user',content:'请为「'+name+'」（日期区间'+range+'，属'+astro+'，核心性格特质：'+trait+'）推演今日（'+d.getFullYear()+'年'+(d.getMonth()+1)+'月'+d.getDate()+'日，周'+'日一二三四五六'.charAt(d.getDay())+'）的运势。请严格贴合该星座特质，避免通用套话。'}],temperature:.8,max_tokens:1500})});
      if(my!==horoSeq)return;/* 用户已切换星座 */
      if(!res.ok)throw new Error('HTTP '+res.status);
      const j=await res.json();
      const txt=(j.choices&&j.choices[0]&&j.choices[0].message&&j.choices[0].message.content)||'';
      if(!txt)throw new Error('月亮没有回应');
      $('horoText').innerHTML=miniMD(txt);
    }catch(e){
      if(my!==horoSeq)return;
      $('horoText').innerHTML='<strong style="color:#ff9d9d">✕ 观星信号中断了…</strong>'+
        '<p style="color:#cbc0ea;margin-top:8px">原因：'+escapeHtml(String(e&&e.message||'未知错误').slice(0,160))+'</p>'+
        '<p style="color:#8a81b8;margin-top:8px">请检查网络后重试。</p>';
    }
  }
  $('btnHoroscope').onclick=()=>{
    $('horoView').style.display='block'; window.scrollTo(0,0); document.body.style.overflow='hidden';
  };
  $('btnBackHoro').onclick=()=>{
    $('horoView').style.display='none';
